...
- Steer an active session with commands like `/model`, `/fast`, `/personality`, `/permissions`, `/agent`, and `/status`

:::tip Queue commands during tasks
...
2. Select the approval preset that matches your comfort level:
   - **Auto** - Hands-off runs
   - **Read Only** - Review all edits before applying
...
2. Optional: provide inline prompt text (e.g., `/plan Propose a migration plan for this service`)
...
:::tip Difference from Ctrl+L
Unlike `Ctrl+L`, `/clear` starts a new conversation. `Ctrl+L` only clears the terminal view and keeps the current chat. Codex disables both actions while a task is in progress.
:::
...
**Expected:** Codex stops all background terminals for the current session.

:::info Alias
`/clean` is still available as an alias for `/stop`.
:::
...
2. Toggle the features you want (e.g., Apps or Smart Approvals), then restart Codex if the prompt asks you to
...
- `rules`
- `enforce_residency`
- `experimental_network` (when configured)

**Use case:** Debug why an effective setting differs from `config.toml`.
...
**Expected:** Codex exits immediately.

:::danger Save your work first
